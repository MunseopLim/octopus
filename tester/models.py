...
class TestStage(models.Model):
    ...
    expect_result = models.TextField(
        max_length=300, help_text="e.g. 5.3.0-24-generic", blank=True
    )
    display_ok_type = models.CharField(
        "How to display result to OK",
        choices=DISPLAY_OK_TYPE_OPTION,
        max_length=100,
        default="TYPE1",
    )
    display_fail_type = models.CharField(
        "How to display result to fail",
        choices=DISPLAY_FAIL_TYPE_OPTION,
        max_length=100,
        default="TYPE1",
    )
    resume_command = models.CharField(
        max_length=300,
        help_text="e.g. uname -r. if test stage failed, this will be shown to resume",
        blank=True,
    )
